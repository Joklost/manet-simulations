...
{
    \filldraw[fill=green!20] (\Node{#1} #2.west) rectangle (\Node{#1} #3.east) node[midway] {$t$};
}

\newcommand{\Listen}[3]
{
    \filldraw[fill=blue!20] (\Node{#1} #2.west) rectangle (\Node{#1} #3.east) node[midway] {$l$};
}

\newcommand{\Sleep}[3]
{
    \filldraw[fill=yellow!20] (\Node{#1} #2.west) rectangle (\Node{#1} #3.east) node[midway] {$s$};
}

\newcommand{\Inform}[2]
{
    \filldraw[fill=gray!20] (\Node{#1} #2.west) rectangle (\Node{#1} #2.east);
}