...
%To facilitate the problems mentioned in \autoref{sec:reachi-data-experiments}, we have developed our own model for computing path loss called. 



\todo[inline]{overview of method}
...
Since ......... hvorfor split i 2\medbreak
...
\begin{eq}
    \mathit{compRSSI}(l)= \alpha \cdot (\ln{d(l)} / \ln{\delta}) + \beta
\end{eq}
...
To compare each iteration, the following score function will be used:
\begin{eq}
    \mathit{score}(\alpha, \beta, \delta) = \sum\limits_{l\ \in\ \mathit{links}} (\mathit{compRSSI}(l) - \mathit{measuredRSSI}(l))^2
\end{eq}
...
\todo[inline]{something about cvpl}
The $\mathit{cvpl}$ had the best score of 0.0372. As the score is for each link,

The $\mathit{bopl}$ however got worse result. The score per link was 0.9302 which is much larger compared to $\mathit{cvpl}$\'s score. It is important to note that there was only 377 links in the training set for the $\mathit{bopl}$ function, which is wastly less compared to the 13481 links for the $\mathit{cvpl}$ function. Another reason for the lower precisision of $\mathit{bopl}$ is that the training set does not only consist of links with 100\% building. This was however necessary as there was close to none links with 100\% building. Having a large set of links with 100\% building would result in higher precision.
