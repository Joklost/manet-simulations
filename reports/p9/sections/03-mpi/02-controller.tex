...
The \texttt{ShouldReceive} function (\autoref{algo:controllershouldreceive}) contains a pseudo code description of the packet error probability from \autoref{sec:pep}. \medbreak
...
% The participants are the entities in the sequence, which in this case is Node1, Node2, Node3 and Controller. An participant is \doublequote{active} when there is a block on their lifeline (the dotted line going downwards). If the participant is not active, it means the participant is not participating anymore, but is not blocked in participating again at a later point in time. An arrow signifies a message. A dotted arrow signifies a response. In this diagram all messages are asynchronous, which can be seen by the fact that the arrow heads are not solid. If the heads where solid, the message would be synchronous.


\begin{enumerate}
    \item[1] All participants initialise.
    \item[2] Node1 will \texttt{Sleep} for 5 seconds.
    \item[3] Node2 and Node3 \texttt{Listen} for 10 seconds.
    \item[4] Node1 will \texttt{Broadcast} a packet \textit{p}, and de-initialises.
    \item[5] Node2 and Node3 receives the packet \textit{p} and de-initialises.
    \item[6] The Controller de-initialises.
\end{enumerate}
    ...
    \Fn{\FController{}}{
        ...
        \Repeat{\textit{protocol terminates}}{
            ...
            \ElseIf{m.action = listen}{ \label{line:action-listen}
                l $\leftarrow$ \{ m.source, m.localtime, m.localtime $+$ m.duration \}\;
            ...
            \ForEach{l $\in$ listens}{ \label{line:iterlistens}
                ...
                \ForEach{t $\in$ transmissions}{ \label{line:itertransmissions}
                    ...
                    \If{t.start $\geq$ l.start \And t.end $\leq$ l.end}{ \label{line:iswithin}
                        \If{\texttt{ShouldReceive}(l.id, t.id, transmissions, |t.data|)}{ \label{line:shouldreceive}
...
\end{algorithm}
\todo[inline]{fix according to feedback}
\begin{algorithm}[ht]
    \DontPrintSemicolon
    ...
    \Fn{\FShouldReceive{n$_r$, m$_t$, transmissions, packetsize}}{
        \tcp{Compute the packet error probability.}
        $P_{I}$ $\leftarrow$ 0\;
        ...
        \ForEach{t $\in$ transmissions}{
            ...
            % P$_{I}$ $\leftarrow$ P$_{I,dB}$ + $10^{\frac{RSSI_{dBm}(n_{r}, m)}{10}}$ \;
        }

        %\leftarrow link\_model()

        $P_p(\text{n}_r, \text{m}_t, ) $
        % \tcp{$P_{N,dB}$ is the noise power from \autoref{sec:pep}.}
        % $P_{N}$ $\leftarrow$ $10^{\frac{P_{N,dB}}{10}}$\;
        % $P_{NI,db}$ $\leftarrow$ $10 \log_{10}(P_N + P_I)$\;
        % $\gamma_{dB}$ $\leftarrow$ $RSSI_{dBm}(n_{r}, m_{t}) - P_{NI,db}$\;
        % $\gamma$ $\leftarrow$ $10^{\frac{\gamma_{dB}}{10}}$\;
        % $P_b$ $\leftarrow$ $\frac{1}{2} \cdot \text{erfc}(\sqrt{\frac{\gamma}{2}})$\;
        % $P_p$ $\leftarrow$ $1 - (1 - P_b)^{\text{packetsize}}$\;

        %$P_{NI,db} = 10 \log_{10}\left( 10^{\frac{P_{N,dB}}{10}} + \mathlarger{\sum}\limits_{m \in nodes_t}  10^{\frac{RSSI_{dBm}(n_{r}, m)}{10}} [m \neq m_{t}] \right) $\;
        \If{$P_p$ = 0.0}{
            \KwRet true\;
        }

        should-receive $\leftarrow$ randomly choose based on the packet error probability $P_p$\;
