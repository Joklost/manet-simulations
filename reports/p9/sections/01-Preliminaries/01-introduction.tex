...
%In addition to the centralised controller, we suggest two methods for  

%, essentially skipping long durations of inactivity


%Another challenge when testing in a real-life scenario is collecting data from the test, e.g., packet loss, colliding transmissions, 
%~\cite{website:reachiproject}
%Testing the capabilities of a wireless network protocol can prove to be a challenge when testing with physical units in a real-life scenario, as problems like scalability and repeatability quickly arise.
...
%When emulating the physical hardware, we are able to both scale and repeat simulations.

%With our library, we emulate the physical devices, and the communication between these using software
